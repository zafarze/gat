# D:\New_GAT\core\views\dashboard.py (ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ)
...
def dashboard_view(request):
    ...
    и новым фильтром по периоду.
    """
    user = request.user
    today = timezone.now().date()

    # --- ЛОГИКА ФИЛЬТРА ПО ПЕРИОДУ ---
    period = request.GET.get('period', 'year') # По умолчанию показываем данные за 'Этот год'
    ...
    base_results_qs = StudentResult.objects.select_related(
    ...
    if not user.is_superuser and hasattr(user, 'profile') and user.profile.role == 'SCHOOL_DIRECTOR':
        if user.profile.school:
            director_school = user.profile.school
            base_results_qs = base_results_qs.filter(student__school_class__school=director_school)

    # --- РАСЧЕТ KPI КАРТОЧЕК ---
    kpi_results_qs = StudentResult.objects.all()
    if start_date and end_date:
        kpi_results_qs = kpi_results_qs.filter(gat_test__test_date__range=(start_date, end_date))
    
    if director_school:
        kpi_results_qs = kpi_results_qs.filter(student__school_class__school=director_school)

    school_count = School.objects.filter(classes__students__results__in=kpi_results_qs).distinct().count()
    ...
    if director_school:
        # --- НОВАЯ ЛОГИКА: График для директора (успеваемость классов) ---
        ...
        for result in base_results_qs:
            ...
            class_performance[class_name]['total_score'] += score
    ...
    # --- НОВАЯ ЛОГИКА: Виджеты "Лидеры" и "В зоне внимания" ---
    ...
    student_performance = []
    student_ids = student_avg_scores.keys()
    students_map = {s.id: s for s in Student.objects.filter(id__in=student_ids).select_related('school_class')}
    ...
    # Тесты без результатов
    tests_qs_for_widget = GatTest.objects.all()
    if director_school:
        tests_qs_for_widget = tests_qs_for_widget.filter(school_class__school=director_school)
    if start_date and end_date:
    ...
    context = {
        ...
        'director_chart_labels': director_chart_labels,
        'director_chart_data': director_chart_data,
