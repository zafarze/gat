# D:\New_GAT\core\views\grading.py (ПОЛНАЯ ВЕРСИЯ С ИСПРАВЛЕНИЕМ ОШИБКИ)
...
def _get_grading_context(get_params, request_user): # Определение функции уже правильное
    """
    Готовит полные данные для страницы Оценок, включая всю логику фильтрации.
    """
        ...
        if first_result:
            ...
            if parent_class_ids:
                ...
                if missing_subject_ids:
                    parent_class_subjects_qs = ClassSubject.objects.filter(school_class_id__in=parent_class_ids, subject_id__in=missing_subject_ids).order_by('subject_id').distinct('subject_id')
                    for cs in parent_class_subjects_qs: q_counts[cs.subject_id] = cs.number_of_questions
        for subj in header_subjects:
            table_headers.append({'subject': subj, 'q_count': q_counts.get(subj.id, 0)})
        if is_total_mode:
            ...
            for data in student_data.values():
                if len(data["tests"]) == 2:
                    grades_by_subject = {}
                    for subject_id, score in data["scores"].items():
                        q_count = q_counts.get(subject_id, 0) * 2
                        if q_count > 0:
                            percentage = (score / q_count) * 100
                            grade = utils.calculate_grade_from_percentage(percentage)
                            grades_by_subject[subject_id] = grade
                        else: grades_by_subject[subject_id] = "—"
                    table_rows.append({'student': data["student"], 'is_total': True, 'grades_by_subject': grades_by_subject, 'total_score': sum(data["scores"].values())})
        else:
            ...
            for result in results_qs.distinct():
                ...
                for subject_id, score in scores_by_subject.items():
                    q_count = q_counts.get(subject_id, 0)
                    if q_count > 0:
                        percentage = (score / q_count) * 100
                        grade = utils.calculate_grade_from_percentage(percentage)
                        grades_by_subject[subject_id] = grade
                    else: grades_by_subject[subject_id] = "—"
                table_rows.append({'student': result.student, 'result_obj': result, 'is_total': False, 'grades_by_subject': grades_by_subject, 'total_score': sum(scores_by_subject.values())})
...
def grading_view(request):
    """Отображает страницу с оценками."""
    # --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
    context = _get_grading_context(request.GET, request.user)
    context['title'] = 'Таблица оценок'
    return render(request, 'grading/grading.html', context)

@login_required
def export_grading_pdf(request):
    """Экспортирует таблицу оценок в PDF."""
    # --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
    context = _get_grading_context(request.GET, request.user)
...
def export_grading_excel(request):
    """Экспортирует таблицу оценок в Excel."""
    # --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
    context = _get_grading_context(request.GET, request.user)
    ...
    # ... остальная логика экспорта в Excel без изменений ...
    workbook = Workbook()
    ...
    header2 = ["", "", "", ""]
    for header_data in table_headers: header2.append("(10 баллов)")
    header2.append("")
