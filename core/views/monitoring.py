# D:\New_GAT\core\views\monitoring.py (ФИНАЛЬНАЯ ВЕРСИЯ С ПРАВАМИ ДОСТУПА)
...
def _get_monitoring_context(get_params, request_user):
    """
    Готовит данные для Мониторинга с фильтрацией по правам доступа.
    """
    ...
    if form.is_valid():
        ...
        if academic_year: results_qs = results_qs.filter(gat_test__quarter__year=academic_year)
        if quarter: results_qs = results_qs.filter(gat_test__quarter=quarter)
        if schools: results_qs = results_qs.filter(student__school_class__school__in=schools)
        ...
        if schools: title_details['schools'] = ", ".join([s.name for s in schools])
        if school_classes: title_details['classes'] = ", ".join([c.name for c in school_classes])
        if quarter: title_details['period'] = str(quarter)
        elif academic_year: title_details['period'] = str(academic_year)

        is_total_mode = gat_tests and '1' in gat_tests and '2' in gat_tests
        if is_total_mode: title_details['test_type'] = "GAT Total"
        elif gat_tests and '1' in gat_tests: title_details['test_type'] = "GAT-1"
        elif gat_tests and '2' in gat_tests: title_details['test_type'] = "GAT-2"
        ...
        if first_result:
            ref_classes = SchoolClass.objects.filter(id__in=results_qs.values_list('student__school_class_id', flat=True).distinct()).select_related('parent')
            class_subjects_qs = ClassSubject.objects.filter(school_class__in=ref_classes, subject__in=header_subjects)
            for cs in class_subjects_qs: q_counts[cs.subject_id] = cs.number_of_questions
            ...
            if parent_class_ids:
                ...
                if missing_subject_ids:
                    parent_class_subjects = ClassSubject.objects.filter(school_class_id__in=parent_class_ids, subject_id__in=missing_subject_ids).order_by('subject_id')
                    for cs in parent_class_subjects:
                        if cs.subject_id not in q_counts: q_counts[cs.subject_id] = cs.number_of_questions

        for subj in header_subjects: table_headers.append({'subject': subj, 'q_count': q_counts.get(subj.id, 0)})
        ...
        if is_total_mode:
            ...
            for r in results_qs.filter(gat_test__test_number__in=[1, 2]):
                if not (r.student and isinstance(r.scores, dict)): continue
                student_data[r.student_id]["student"] = r.student
                student_data[r.student_id]["tests"].add(r.gat_test.test_number)
                for sid, answers in r.scores.items(): student_data[r.student_id]["scores"][int(sid)] += sum(answers)
            for data in student_data.values():
                if len(data["tests"]) == 2:
                    total_score = sum(data["scores"].values())
                    table_rows.append({'student': data["student"], 'is_total': True, 'scores_by_subject': data["scores"], 'total_score': total_score})
        else:
            if gat_tests: results_qs = results_qs.filter(gat_test__test_number__in=gat_tests)
            for result in results_qs.distinct():
                if not (result.student and isinstance(result.scores, dict)): continue
                scores_by_subject = {int(sid): sum(ans) for sid, ans in result.scores.items()}
                table_rows.append({'student': result.student, 'result_obj': result, 'is_total': False, 'scores_by_subject': scores_by_subject, 'total_score': sum(scores_by_subject.values())})
        
        table_rows.sort(key=lambda x: x.get('total_score', 0), reverse=True)

    return {'form': form, 'table_headers': table_headers, 'table_rows': table_rows, 'has_results': bool(get_params), 'title_details': title_details}

@login_required
def monitoring_view(request):
    """Отображает страницу мониторинга."""
    context = _get_monitoring_context(request.GET, request.user)
...
def export_monitoring_excel(request):
    ...
    for header_data in table_headers: header1.append(header_data['subject'].abbreviation or header_data['subject'].name)
    header1.append("Общий балл")
    sheet.append(header1)
    header2 = ["", "", "", ""]
    for header_data in table_headers: header2.append(f"({header_data['q_count']})")
    header2.append("")
    ...
    for i, row_data in enumerate(table_rows, 1):
        row = [i, str(row_data['student']), str(row_data['student'].school_class), "GAT Total" if row_data['is_total'] else row_data['result_obj'].gat_test.name]
