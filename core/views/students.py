# D:\New_GAT\core\views\students.py (ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ)
...
from .. import utils # Импортируем наш файл с помощниками
...
# --- CRUD (Create, Read, Update, Delete) для учеников ---
...
class StudentListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if school_id:
            queryset = queryset.filter(school_class__school_id=school_id)
        if class_id:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'error' in report:
                ...
                messages.success(request, f"Обработка завершена. Всего обработано: {report['processed_count']}. Создано новых: {report['created_count']}. Обновлено существующих: {report['updated_count']}.")
                if report['errors']:
...
# --- Страница детальной аналитики по ученику ---

def _get_grade_and_subjects_performance(result, subject_map):
    """Вспомогательная функция: Рассчитывает оценку, лучший и худший предметы."""
...
def student_progress_view(request, student_id):
    """Отображает страницу прогресса студента с аналитикой и заметками."""
    student = get_object_or_404(Student.objects.prefetch_related('notes__author'), id=student_id)
    ...
    if not student_results_qs:
        return render(request, 'students/student_progress.html', {
            'title': f'Аналитика студента: {student}',
            'student': student, 'detailed_results_data': [], 'notes': student.notes.all()
        })
